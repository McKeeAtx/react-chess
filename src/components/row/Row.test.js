...
describe('<Row />', () => {
  ...
  it('each <Row /> has a label', () => {
    const wrapper = shallow(<Row row={0} gameState={GameState.initialBoard()} squareClicked={jest.fn()} />);
  ...
  it('the label of the top <Row /> is 8', () => {
    const wrapper = shallow(<Row row={7} gameState={GameState.initialBoard()} squareClicked={jest.fn()} />);
  ...
  it('properties are passed from top <Row /> to <Square />s as expected', () => {
    ...
    const wrapper = shallow(<Row row={7} gameState={GameState.initialBoard()} squareClicked={mockFn} />);
  ...
  it('properties are passed from bottom <Row /> to <Square />s as expected', () => {
    const mockFn = jest.fn();
    const wrapper = shallow(<Row row={0} gameState={GameState.initialBoard()} squareClicked={mockFn} />);
    expect(wrapper.find(Square).at(0).props()).toEqual({
  ...
  it('the last header row contains column names', () => {
    const mockFn = jest.fn();
    const wrapper = shallow(<Row row='header' gameState={GameState.initialBoard()} squareClicked={mockFn} />);
