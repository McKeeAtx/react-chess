...
describe('knight' , () => {
    ...
    it('white knight on B1 can move to C3 and A3' , () => {
        ...
        expect(gameState.getAllowedSquares(B1)).toEqual([C3, A3]);
    });

    it('white knight on G1 can move to H3 and F3' , () => {
        let gameState = GameState.initialBoard();
        expect(gameState.getAllowedSquares(G1)).toEqual([H3, F3]);
    });

    it('black knight on B8 can move to C6 and A6' , () => {
        let gameState = GameState.initialBoard();
        expect(gameState.getAllowedSquares(B8)).toEqual([C6, A6]);
    });

    it('black knight on G8 can move to H6 and F6' , () => {
        let gameState = GameState.initialBoard();
        expect(gameState.getAllowedSquares(G8)).toEqual([H6, F6]);
    });
